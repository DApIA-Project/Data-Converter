    ...
    context('when CSV drone data are valid', () => {
        ...
        it('returns SBS message', async () => {
            assert.deepStrictEqual(
                ...
                `MSG,3,1,1,${icao24},1,2023/10/02,10:00:01.000,2023/10/02,10:00:01.000,${name},${positionAltitude},${groundSpeed},${heading},${positionLatitude},${positionLongitude},,,0,,0,0,{"fixName":"${fixName}","significantPoint":"${significantPoint}","timeElapsed (s)":"${timeElapsed}","altitudeMax (ft)":"${altitudeMax}","airSpeed (kt)":"${airSpeed}","cas (kt)":"${cas}","mach":"${mach}","distanceToNextWaypoint (NM)":"${distanceToNextWaypoint}","flownDistance (NM)":"${flownDistance}","wind.eastward (kt)":"${windEastward}","wind.northward (kt)":"${windNorthward}","wind.upward (ft/min)":"${windUpward}","route (deg)":"${route}","mass (kg)":"${mass}","isOneWay":"${isOneWay}"}`,
            )
        })
        ...
        it('uses CSV drone extra field as SBS message properties if matching', async () => {
            assert.deepStrictEqual(
                ...
                `SEL,2,3,4,${icao24},5,2023/10/02,10:00:01.000,2023/10/02,10:00:01.000,${name},${positionAltitude},${groundSpeed},${heading},${positionLatitude},${positionLongitude},,,0,1,0,0,{"fixName":"${fixName}","significantPoint":"${significantPoint}","timeElapsed (s)":"${timeElapsed}","altitudeMax (ft)":"${altitudeMax}","airSpeed (kt)":"${airSpeed}","cas (kt)":"${cas}","mach":"${mach}","distanceToNextWaypoint (NM)":"${distanceToNextWaypoint}","flownDistance (NM)":"${flownDistance}","wind.eastward (kt)":"${windEastward}","wind.northward (kt)":"${windNorthward}","wind.upward (ft/min)":"${windUpward}","route (deg)":"${route}","mass (kg)":"${mass}","isOneWay":"${isOneWay}"}`,
            )
        })
